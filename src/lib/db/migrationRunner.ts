...
function isSchemaAlreadyApplied(
  ...
  switch (migration.version) {
    ...
    case "187":
      // Fork migration renamed from 164 after upstream shipped
      // 164_retire_microsoft_designer_web (version collision). Fork databases that
      // already applied it under the old number already have the table; the ledger
      // rewrite itself is registered in RENAMED_MIGRATION_COMPATIBILITY.
      return hasTable(db, "patty_settlement_outbox");
    default:
      return false;
  }
}
