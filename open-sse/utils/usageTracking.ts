 ...
 */
/**
 * Return shape of normalizeResponsesUsageToOpenAI: the canonical OpenAI usage
 * fields plus the Responses-style aliases defensive readers may probe for.
 */
interface NormalizedResponsesUsage extends UsageData {
  input_tokens?: number;
  output_tokens?: number;
...
/**
 * Wire shape accepted by normalizeResponsesUsageToOpenAI: Responses-style fields
 * with OpenAI-style fallbacks. All optional — the reader tolerates any subset.
 */
...
export function normalizeResponsesUsageToOpenAI(
  usage: ResponsesUsageWire | null | undefined
): NormalizedResponsesUsage | null {
  ...
  const normalized: NormalizedResponsesUsage = {
