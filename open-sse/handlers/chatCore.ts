...
import {} from "../services/reasoningCache.ts";
...
import { incrementRequestCount, incrementTokenUsage } from "../services/geminiRateLimitTracker.ts";
...
export async function handleChatCore({
  ...
  // Fork→upstream (#3501 extraction): thinking normalization (#3554), adaptive/haiku
  // collapse, per-model default reasoning_effort (#6879/#7694), MiMo shape, the
  // unsupported-param strip, and the GPT-5 sampling guard now all live in
  // chatCore/upstreamBody.ts::prepareUpstreamBody — the inline copies that used to
  // live here double-applied on the leg path and leaked a replaced model's suffix
  // effort (suffix-effort-propagation regression).
  ...
  // Unsupported-param strip + GPT-5 sampling guard: also owned by
  // chatCore/upstreamBody.ts::prepareUpstreamBody (see note above).
  ...
  // leak (GHSA-6c7w-56xp-wpc6). Fork delta: chatgpt-web excluded from dedup —
  // its browser transport is session-stateful, so identical requests must not
  // share one response object.
  ...
  if (!stream) {
    try {
      ...
      if (pattyDecision) {
        ...
        // Patty echo wins over the #1311 alias echo: the client must see the
        // public (settlement-billed) model name, never the upstream id.
